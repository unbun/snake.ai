...
class EvolutionaryAlgorithm:
    ...
    def __init__(self, actions, discount, featureExtractor, strategies, grid_size, max_game_iter, sigma=0.1, alpha=0.1,
                 mode=0, population_size=10, filename=None):
        ...
        self.weights = np.random.rand(self.n_actions, self.featureExtractor.nFeatures()) / np.sqrt(
    ...
    def _sample_candidates(self):
        return [self.weights + (self.sigma * np.random.randn(*self.weights.shape)) for _ in
...
class ESAgent:
    ...
    def getAction(self, state):
        ...
        abs_action = move.Move(self.featureExtractor.toAbsolutePos(state, rel_action.direction()),
                               norm=rel_action.norm())
        return abs_action
...
def es_strategy(strategies, featureExtractor, discount, grid_size, num_trials=100, max_iter=1000,
                filename="es/weights1.p", verbose=False):
    es_id = len(strategies)
    actions = lambda s: s.all_rel_actions(es_id)

    ea = EvolutionaryAlgorithm(actions, discount, featureExtractor, strategies, grid_size, max_game_iter=max_iter,
                               sigma=0.001, alpha=0.001, mode=1, population_size=20, filename="simple-pg-r6.p")
    ea.train(num_trials)
    ...
    with open("info/{}txt".format(filename[:-1]), "wb") as fout:
        ...
        print("feature radius: ", featureExtractor.radius, file=fout)
        print("grid: {}, trials: {}, max_iter: {}".format(grid_size, num_trials, max_iter), file=fout)
        print("discount: {}".format(discount), file=fout)
